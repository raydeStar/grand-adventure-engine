...
### 1. ~~Wiki-to-Narrator Knowledge Pipeline~~ *(Wiki.js removed — knowledge now served by ContentRegistry + YAML lore seeds)*
- Extended `IWikiService` with `SearchAsync` and `GetPagesAsync` (GraphQL read operations)
- Implemented both in `WikiService.cs`
- Created `WorldKnowledgeBuilder` — pre-fetches room/NPC/faction/region lore from the content registry
- Wired into all four narrator methods (NarrateAction, ProcessFreeForm, ProcessConversation, ProcessCombat)
- Added lore-seed loading at startup (world overview, regions, factions, starting room)

### 2. NPC Knowledge Scoping
- Added `KnowledgeScopes` field to `Npc` model — list of tags controlling lore access
- Created `BuildScopedContextAsync` in WorldKnowledgeBuilder — filters lore entries by NPC scopes
...
src/GAE.Narrator/NarratorService.cs            — Lore knowledge injection in all prompts, disposition helpers, NPC generation defaults
src/GAE.Engine/GameEngine.cs                   — ApplyInteractionUpdate handles rich disposition
src/GAE.Dashboard.Api/Program.cs               — DI registration, lore-seed loading, LoreNpc DTO expanded
...
### ~~1. NPC Wiki Auto-Publishing~~ *(Obsolete — Wiki.js removed)*
~~When `GenerateNpcAsync` or `GenerateRoomAsync` creates a new NPC, write their page to the wiki:~~
```csharp
// No longer applicable — NPC data lives in the content registry
```
...
### ~~3. Story Event Wiki-Writing~~ *(Obsolete — Wiki.js removed; use lore entries instead)*
...
The narrator's SearchContextAsync will find lore entries when relevant topics come up.
...
- **WorldKnowledgeBuilder is nullable** in NarratorService — if the content registry is empty, narrator still works, just without lore context.
