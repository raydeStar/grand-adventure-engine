...
public class NarratorService : INarratorService
{
    ...
    public async Task<string> NarrateActionAsync(NarratorContext context, CancellationToken ct = default)
    {
        var systemPrompt = """
            ...
            Keep responses to 2-4 sentences. Use vivid sensory detail.
        ...
        var userPrompt = $"""
            ...
            Mechanical Result: {context.MechanicalResult.MechanicalSummary}
            """;
    ...
    private async Task<string> CompletionAsync(string systemPrompt, string userPrompt, CancellationToken ct)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync("v1/chat/completions", request, _jsonOptions, ct);
            response.EnsureSuccessStatusCode();
            var result = await response.Content.ReadFromJsonAsync<LmStudioResponse>(_jsonOptions, ct);
            return result?.Choices?.FirstOrDefault()?.Message?.Content ?? FallbackNarration();
        }
        catch (Exception ex)
        {
